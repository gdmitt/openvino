...
void Snippet::execute(dnnl::stream strm) {
    ...
    for (auto i = 0; i < srcMemPtrs[0]->getDesc().getShape().getElementsCount(); i++) {
        std::cout << std::to_string(float(static_cast<int8_t*>(srcMemPtrs[0]->GetData())[i])) << " ";
    }
    std::cout << std::endl;
    for (auto i = 0; i < srcMemPtrs[1]->getDesc().getShape().getElementsCount(); i++) {
        std::cout << std::to_string(float(static_cast<int*>(srcMemPtrs[1]->GetData())[i])) << " ";
    }
    std::cout << std::endl;
    for (auto i = 0; i < srcMemPtrs[2]->getDesc().getShape().getElementsCount(); i++) {
        std::cout << std::to_string(float(static_cast<float*>(srcMemPtrs[2]->GetData())[i])) << " ";
    }
    std::cout << std::endl;
    ...
    for (auto i = 0; i < dstMemPtrs[0]->getDesc().getShape().getElementsCount(); i++) {
        std::cout << std::to_string(static_cast<float>(static_cast<int8_t*>(dstMemPtrs[0]->GetData())[i])) << " ";
    }
    std::cout << std::endl;
    for (auto i = 0; i < dstMemPtrs[1]->getDesc().getShape().getElementsCount(); i++) {
        std::cout << std::to_string(static_cast<float>(static_cast<float*>(dstMemPtrs[1]->GetData())[i])) << " ";
    }
    std::cout << std::endl;
}
