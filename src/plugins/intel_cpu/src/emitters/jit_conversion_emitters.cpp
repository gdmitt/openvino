...
jit_convert_truncation_emitter::jit_convert_truncation_emitter(jit_generator *host, cpu_isa_t host_isa, const std::shared_ptr<ngraph::Node>& node, Precision exec_prc)
...
void jit_convert_truncation_emitter::emit_isa(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    ...
    switch (output_type) {
        case ov::element::f32:
            ...
            break;
        case ov::element::i32:
            if (one_of(input_type, ov::element::f32, ov::element::bf16)) {
        ...
        case ov::element::i8:
        case ov::element::u8:
            if (!one_of(input_type, ov::element::i32, ov::element::u8, ov::element::i8)) {
                h->uni_vcvttps2dq(vmm_src, vmm_src);
            }
...
void jit_convert_truncation_emitter::dword2int8(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    ...
    if (isa == dnnl::impl::cpu::x64::avx512_common) {
