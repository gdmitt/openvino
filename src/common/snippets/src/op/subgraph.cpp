...
Shape snippets::op::Subgraph::canonicalize(const BlockedShapeVector& outputShapes, const BlockedShapeVector& inputShapes) {
    ...
    // work with precisions. Firstly we should insert Converts after parameters and before results without validate_nodes_and_infer_types() call
    // because there may be Converts with not supported destination type inside body. Thus, after that we should propagate precision inside body
...
void insertConvertSaturationAfterNode(const std::shared_ptr<Node>& node, const ov::element::Type element_type, const set<size_t>& needed_ports = {}) {
    const std::set<size_t> default_port = { 0 };
    set<size_t> ports = needed_ports;
    if (ports.size() == 0) {
        std::vector<size_t> ps(node->get_output_size());
        std::iota(ps.begin(), ps.end(), 0);
        ports.insert(ps.begin(), ps.end());
    }

    for (auto port : ports) {
        for (auto consumer : node->get_output_target_inputs(port)) {
            if (auto existing_convert_t = ngraph::as_type_ptr<ngraph::snippets::op::ConvertTruncation>(consumer.get_node()->shared_from_this())) {
                if (existing_convert_t->get_destination_type() != element_type) {
                    insertConvertSaturationAfterNode(existing_convert_t, element_type, default_port);
                }
                continue;
            }

            auto existing_convert_s = ngraph::as_type_ptr<ngraph::snippets::op::ConvertSaturation>(consumer.get_node()->shared_from_this());
            if (!existing_convert_s && !ov::is_type<ngraph::op::v0::Result>(consumer.get_node()->shared_from_this()) && consumer.get_node()->shared_from_this()->get_element_type() != element_type ||
...
void snippets::op::Subgraph::align_precision(const BlockedShapeVector& outputShapes, const BlockedShapeVector& inputShapes) {
    ...
    ov::pass::Serialize("C:\\openvino\\graphs\\0_body.xml", "C:\\openvino\\graphs\\0_body.bin").run_on_model(m_body);
    ...
    for (size_t i = 0; i < outputShapes.size(); i++) {
        ...
        // If there is real Convert from graph (ConvertTruncation) in the end we should check destination type and add ConvertSaturation after that if needed
        if (auto existing_convert_t = ngraph::as_type_ptr<ngraph::snippets::op::ConvertTruncation>(body_results[i]->get_input_node_shared_ptr(0))) { // common convert? TODO
            ...
            if (original_input_element_type != supported_exec_type) {
                insertConvertSaturationAfterNode(existing_convert_t->get_input_node_shared_ptr(0), original_input_element_type,
                                                 {existing_convert_t->get_input_source_output(0).get_index()});
            }
        } else {
    ...
    ov::pass::Serialize("C:\\openvino\\graphs\\1_body_precision.xml", "C:\\openvino\\graphs\\1_body_precision.bin").run_on_model(m_body);
}
...
void snippets::op::Subgraph::convert_to_snippet_dialect() {
    ...
    ov::pass::Serialize("C:\\openvino\\graphs\\2_body_dialect.xml", "C:\\openvino\\graphs\\2_body_dialect.bin").run_on_model(m_body);
}
...
snippets::Schedule snippets::op::Subgraph::generate(ngraph::pass::Manager& opt, const void* compile_params) {
    ...
    ov::pass::Serialize("C:\\openvino\\graphs\\3_body_cpu.xml", "C:\\openvino\\graphs\\3_body_cpu.bin").run_on_model(m_body);
